...
class CompanyDriversController extends Controller
{
    ...
    public function index(){
    	$drivers = Driver::select("*")->where("companies_id","=",auth('admin')->user()->id)->with("driver_location")->paginate(10);
        if($drivers->count()<=0){
            return view("pages.undefind",["title"=>"Undefind","message"=>"Undefind Drivers"]);
        }
    ...
    public function editDriver($id){
        $trucks_types = DB::table('trucks_types')->get();
        $driver = Driver::select("*")->where("companies_id","=",auth('admin')->user()->id)->where("id","=",$id)->get()->first();
        if($driver == null){
    ...
    public function search(Request $req,$name=null)
    {
        ...
        /*if($req->ajax())
        {
            if($name != null){
                ...
                $drivers = Driver::select("id","name")->where("companies_id","=",auth('admin')->user()->id)->where('name','like',"%".$name."%")->get();
            ...
            if($name != null){
                ...
                $drivers = Driver::select("*")->with("driver_location")->where("companies_id","=",auth('admin')->user()->id)->where('name','like',"%".$name."%")->paginate(10);
    ...
    {
        $drivers = Driver::select("*")->where("companies_id","=",auth('admin')->user()->id)->where("drivers.is_active","=",1)->with("driver_location")->paginate(10);
        if($drivers->count()<=0){
            return view("pages.undefind",["title"=>"Undefind","message"=>"Undefind Drivers"]);
        }
    ...
    {
        $drivers = Driver::select("*")->where("companies_id","=",auth('admin')->user()->id)->where("drivers.is_active","=",0)->with("driver_location")->paginate(10);
        if($drivers->count()<=0){
            return view("pages.undefind",["title"=>"Undefind","message"=>"Undefind Drivers"]);
        }
